    ...
    public partial class MainWindow : Window
    {
       ...
       private bool _pageEventsHooked;
        ...
        private bool InitLogic()
        {
            ...
            _initComplete = true;
            EnsureFinalLoadQueued();
            return true;
        }

        private void EnsureFinalLoadQueued() => _startupCoordinator?.EnsureFinalLoadQueued();
        ...
        private async Task FinalLoad()
        {
            ...
            bool needsUpdateLogic = _config.Fields.Elpis_Version == null || _config.Fields.Elpis_Version < ver;

            if (needsUpdateLogic)
            {
                _loadingPage.UpdateStatus("Running update logic...");

                string oldVer = _config.Fields.Elpis_Version?.ToString() ?? "unknown";
            ...
            {
                _loadingPage.UpdateStatus("Signing in...");
        ...
        {
            // Determine if we should minimize instead of closing.
            bool shouldMinimizeToTray = !_forceClose && _config?.Fields.Elpis_MinimizeToTray == true && !_restarting;

            if (shouldMinimizeToTray)
            {
                WindowState = WindowState.Minimized;
                // The Hide() and ShowInTaskbar = false logic is handled in the Window_StateChanged event.
                e.Cancel = true; // Prevents the application from closing.
                return;
            }
           ...
           // }
            ...
            // --- Proceed with a full shutdown ---

            // Use a re-entrancy guard to ensure this logic runs only once.
            if (Interlocked.Exchange(ref _closing, 1) == 1)
            {
                e.Cancel = true; // Prevent another close event while already closing.
                return;
            }
            ...
            try
            {
                ...
                // 2. Dispose the notification icon.
                _notify?.Dispose();
                _notify = null;

                // 3. Persist final application state.
                if (_config != null)
                {
            ...
            {
                Debug.WriteLine("Exception during shutdown: " + ex);
                // Swallow exceptions on shutdown to ensure the app closes.
            }
